...
export class CurrenciesComponent {
  ...
  constructor(public comp: MarketComponent) { }
  ...
  async getCurrencyData() {
    ...
    const currencieslist = ['BTC', 'ETH', 'BNB', 'ADA', 'SOL', 'XRP',
      'DOT', 'DOGE', 'LUNA', 'UNI', 'AVAX', 'LINK', 'ALGO', 'LTC', 'BCH', 'WBTC',
      'MATIC', 'AXS', 'ATOM', 'ICP', 'FIL', 'XTZ', 'XLM', 'VET', 'FTT', 'ETC', 'TRX', 'DAI']
    const currenciesnames = ['Bitcoin', 'Ethereum', 'Binance', 'Cardano', 'Solana',
      'XRP', 'Polkadot', 'Dogecoin', 'Terra', 'Uniswap', 'Avalanche',
    ...
    let json = await fetch('https://min-api.cryptocompare.com/data/pricemultifull?fsyms=' +
      'BTC,ETH,BNB,ADA,SOL,XRP,DOT,DOGE,LUNA,UNI,AVAX,LINK,ALGO,LTC,BCH,WBTC,' +
      'MATIC,AXS,ATOM,ICP,FIL,XTZ,XLM,VET,FTT,ETC,TRX,DAI&tsyms=USD').then(res => res.json())

    let jsonArray: any = Object.values(await json), performers: any = [], i = 0
    ...
    subject.next({
      ...
      "subs":
        ["5~CCCAGG~BTC~USD", "5~CCCAGG~ETH~USD", "5~CCCAGG~BNB~USD", "5~CCCAGG~ADA~USD",
          "5~CCCAGG~SOL~USD", "5~CCCAGG~XRP~USD",
          "5~CCCAGG~DOT~USD", "5~CCCAGG~DOGE~USD", "5~CCCAGG~LUNA~USD", "5~CCCAGG~UNI~USD",
          "5~CCCAGG~AVAX~USD", "5~CCCAGG~LINK~USD", "5~CCCAGG~ALGO~USD",
